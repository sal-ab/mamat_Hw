...
packet_list=$(cat /dev/stdin | sed -e 's/#.*//' | tr -d ' ')
	while read rule; do
		rule_trimmed=$(echo "$rule" | tr -d ' ' | tr -d ' ,' | sed -e 's/#.*//' | tr [:upper:] [:lower:] | \
		...
		if [[ "$rule_trimmed" != "" ]]; then
			...
			dst_port_rule=$(echo $rule_trimmed | awk -F, '{print $4}')
				...
				./firewall.exe "$src_ip_rule" | ./firewall.exe "$dst_ip_rule" | \
				./firewall.exe "$src_port_rule" | ./firewall.exe "$dst_port_rule" )
